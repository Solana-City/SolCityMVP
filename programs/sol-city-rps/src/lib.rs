// JoKenPo (Rock-Paper-Scissors) wager challenges for Solana City.
//
// Ported from MagicBlock's reference template
// (magicblock-labs/magicblock-engine-examples/rock-paper-scissor) almost
// verbatim — the account/instruction model and Private Ephemeral Rollup
// permission flow are privacy-critical, so we keep them unchanged rather
// than risk a subtle bug while adapting. Only the program name/id and a
// few doc comments differ from the source template.
...
declare_id!("C6gWf1CtwgiZKpkmBjjnos13VGfFr3zwuhtkNLkXU5qW");
...
pub mod sol_city_rps {
    ...
    /// `join_game` time, and signs the CPI via seeds derived from `account_type`.
