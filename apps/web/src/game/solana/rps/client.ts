/**
 * JoKenPo on-chain client. Ported from MagicBlock's rock-paper-scissor
 * reference (`lib/rps.ts`) with minimal changes: every identity (the human
 * player's existing Solana City session key, or the JoKenPo Master NPC's
 * local "bot" keypair — see ./bot.ts) is a real local `Keypair`, so a single
 * `RpsClient` shape covers both without a separate burner/wallet-adapter
 * split. The reference's own host/joiner burner-keypair pattern is exactly
 * this same trick; we just reuse Solana City's existing session key instead
...
import {
  ...
  MAGIC_PROGRAM_ID,
  type Member,
} from "@magicblock-labs/ephemeral-rollups-sdk-rps";
import rawIdl from "./idl/sol_city_rps.json";
import type { SolCityRps } from "./idl/sol_city_rps";
import {
  BASE_ENDPOINT,
  TEE_ENDPOINT,
  TEE_WS_ENDPOINT,
...
export const winnerKey = (game: GameAccount): PublicKey | null => {
  ...
  return (game.roundResult as { winner: Record<string, PublicKey> }).winner["0"];
};

const idl = { ...(rawIdl as object), address: PROGRAM_ID.toBase58() } as SolCityRps;
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

/** Minimal Anchor-`Wallet`-shaped adapter around a raw local Keypair. */
function walletAdapterFrom(keypair: Keypair) {
  return {
...
/**
 * One client per identity (human player's session key, or the bot's local
 * keypair): a base-layer connection plus a lazily-authenticated TEE/ER
 * connection — all ER reads/writes go through this identity's own auth
 * token, which is what keeps the opponent's choice hidden.
 */
export class RpsClient {
  readonly keypair: Keypair;
  readonly baseConnection: Connection;
  readonly program: Program<SolCityRps>;
  ...
  constructor(keypair: Keypair) {
    ...
    this.program = new Program<SolCityRps>(idl, provider);
  }
  ...
  choicePda(gameId: BN, player: PublicKey): PublicKey {
    return PublicKey.findProgramAddressSync(
      [Buffer.from(PLAYER_CHOICE_SEED), gameId.toArrayLike(Buffer, "le", 8), player.toBuffer()],
      this.program.programId
    )[0];
  }
  ...
  async teeConn(): Promise<Connection> {
    ...
    const cacheKey = `sol-city-rps-tee-token-${this.publicKey.toBase58()}`;
    ...
    this.teeConnection = new Connection(`${base}?token=${auth.token}`, {
  ...
  async createGameAndDelegate(gameId: BN, stakeLamports: BN, targetWins: number): Promise<string> {
    ...
    const delegateChoiceIx = await this.program.methods
      .delegatePda({ playerChoice: { gameId, player: this.publicKey } } as never)
      .accountsPartial({
  ...
  async joinGameAndDelegate(gameId: BN): Promise<string> {
    ...
    const delegateGameIx = await this.program.methods
      .delegatePda({ game: { gameId } } as never)
      .accountsPartial({ payer: this.publicKey, validator: ER_VALIDATOR, pda: this.gamePda(gameId) })
      .instruction();
    const delegateChoiceIx = await this.program.methods
      .delegatePda({ playerChoice: { gameId, player: this.publicKey } } as never)
      .accountsPartial({
  ...
  private initPermissionIx(gameId: BN, target: "game" | "choice", members: Member[]) {
    const account = target === "game" ? this.gamePda(gameId) : this.choicePda(gameId, this.publicKey);
    const accountType =
      target === "game" ? { game: { gameId } } : { playerChoice: { gameId, player: this.publicKey } };
    return this.program.methods
      .initPermission(accountType as never, members)
      .accountsPartial({
  ...
  async initGameAndOwnChoicePermissions(gameId: BN, player1: PublicKey): Promise<string> {
    const choicePda = this.choicePda(gameId, this.publicKey);
  ...
  async makeChoice(gameId: BN, choice: ChoiceName): Promise<string> {
    const ix = await this.program.methods
      ...
      .accountsPartial({ playerChoice: this.choicePda(gameId, this.publicKey), player: this.publicKey })
      .instruction();
  ...
  async isOnBase(gameId: BN): Promise<boolean> {
    const info = await this.baseConnection.getAccountInfo(this.gamePda(gameId)).catch(() => null);
  ...
  async fetchMyChoiceEr(gameId: BN): Promise<ChoiceName | null> {
    const conn = await this.teeConn();
    const info = await conn.getAccountInfo(this.choicePda(gameId, this.publicKey));
    if (!info) return null;
    const decoded = this.program.coder.accounts.decode("playerChoice", info.data) as {
      choice: Record<string, object> | null;
    };
  ...
  private async revealTx(gameId: BN, player1: PublicKey, player2: PublicKey): Promise<Transaction> {
   ...
   */
  async tryReveal(gameId: BN, player1: PublicKey, player2: PublicKey): Promise<string | null> {
    const conn = await this.teeConn();
  ...
  async nextRound(gameId: BN, player1: PublicKey, player2: PublicKey): Promise<string> {
    const gamePda = this.gamePda(gameId);
    const p1Choice = this.choicePda(gameId, player1);
    const p2Choice = this.choicePda(gameId, player2);
    const ix = await this.program.methods
  ...
  async undelegateAll(gameId: BN, player1: PublicKey, player2: PublicKey): Promise<string> {
    const ix = await this.program.methods
      .undelegateAll()
      .accountsPartial({
        payer: this.publicKey,
  ...
  /** Base layer (after undelegate): pay the winner. Idempotent via `paid`. */
  async claimPot(gameId: BN, player1: PublicKey, player2: PublicKey): Promise<string> {
    const ix = await this.program.methods
      .claimPot()
      .accountsPartial({
        game: this.gamePda(gameId),
  ...
  async cancelGame(gameId: BN): Promise<string> {
    const ix = await this.program.methods
      .cancelGame()
      .accountsPartial({ game: this.gamePda(gameId), vault: this.vaultPda(gameId), player1: this.publicKey })
      .instruction();
    return this.sendBase([ix]);
  }
}
